...
def run_setup_wizard(system, output_path="config.yaml"):
    """
    CLI wizard that detects attached cameras and writes a starter config.yaml.
    Run with:  python multiAcquisition.py --setup

    The wizard asks for:
      - Which cameras to use (by index)
      - A friendly name and chamber label for each
      - The Arduino port and baud rate
      - A save directory

    It assigns 3 consecutive analog pins per chamber (start, stop, spare) and
    populates the ttl_map accordingly.
    """
    print("\n" + "="*60)
    ...
    for i in selected:
        ...
        # Assign 3 consecutive analog pins per chamber: start, stop, spare.
        # pin_base ensures non-overlapping pin sets across chambers.
        pin_base  = i * 3
        start_pin = f"A{pin_base}"
        stop_pin  = f"A{pin_base + 1}"
        spare_pin = f"A{pin_base + 2}"

        print(f"  Default pins: {start_pin}=start, {stop_pin}=stop, {spare_pin}=spare")
        print(f"  TTL numbers: {pin_base+1} (start), {pin_base+2} (stop), {pin_base+3} (spare)")

        ttl_map_entries.append({
            "pin": start_pin, "ttl_label": f"{ch}_start",
            "chamber": ch, "action": "start_recording",
            "arduino": {"port": port, "baud": baud},
        })
        ttl_map_entries.append({
            "pin": stop_pin, "ttl_label": f"{ch}_stop",
            "chamber": ch, "action": "stop_recording",
            "arduino": {"port": port, "baud": baud},
        })
        ttl_map_entries.append({
            "pin": spare_pin, "ttl_label": f"{ch}_event",
            "chamber": ch, "action": "log_event",
            "arduino": {"port": port, "baud": baud},
        })

    save_dir = input("\nSave directory [./recordings]: ").strip() or "./recordings"
    # Assemble the full config dict and write it to YAML.
    config = {
        "save_dir": save_dir,
        "cameras":  cameras_cfg,
